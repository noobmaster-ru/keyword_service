...
class ParseWbSiteClass:
    ...
    async def parse_grade(self, session: aiohttp.ClientSession, nm_id: str) -> int | str:
        async with self.SEMAPHORE:
            try:
                params = {"curr": "RUB"}
    ...
    async def parse_first_page(self, session: aiohttp.ClientSession, keyword: str, articles: dict)-> dict:
        async with self.SEMAPHORE:
            try:
                headers = self.HEADERS_PARSE_PAGE_TEMPLATE.copy()
                    ...
                    for product in products:
                        try:
                            articles[product['id']] = {
                                ...
                                "price": await self.fetch_price(session, str(product['id'])), # product["sizes"][0]["price"]["product"]//100,
    ...
    async def parse_page_number_(self, session: aiohttp.ClientSession, keyword: str, page_number: str, articles: dict) -> dict:
        async with self.SEMAPHORE:
            try:
                headers = self.HEADERS_PARSE_PAGE_TEMPLATE.copy()
