    ...
    def get_list_of_feedback_data(nm_id: str, feedbacks_list: list) -> list:
        ...
        for index, feedback in enumerate(feedbacks_list):
            ...
            if int(feedback_nm_id) == int(nm_id) and count < 5:
                ...
                if text_of_feedback == "":
                    text_of_feedback = feedback.get("text") or feedback.get("cons") or feedback.get("pros")
    ...
    async def parse_last_five_feedbacks_rating(
        ...
        try:
            ...
            async with session.get(
                f"https://basket-{basket}.wbbasket.ru/vol{str(int_nm_id // 100000)}/part{str(int_nm_id // 1000)}/{str(nm_id)}/info/ru/card.json", headers=headers
            ) as response:
